...
E-Mail-Benachrichtigungssystem für Covina Backend:
- SMTP-Integration
- Template-basierte E-Mails
- Benachrichtigungen für Job-Status
- Fehler-Alerts
    ...
    def _load_default_templates(self) -> Dict[str, MailTemplate]:
        ...
        return {
            "job_completed": MailTemplate(
                subject="✅ Covina Job abgeschlossen - {job_id}",
                html_body="""
                <html>
                <head>
                    <style>
                ...
                <body>
                    <div class="header">
                        <h2>🎉 Dokumentenverarbeitung erfolgreich abgeschlossen!</h2>
                    </div>
                    ...
                    <div class="content">
                        ...
                        <div class="metrics">
                            <h3>📊 Verarbeitungsstatistiken</h3>
                            <ul>
                                ...
                                <li><strong>KI-Entitäten gefunden:</strong> {ai_entities}</li>
                                <li><strong>Metadata-Vollständigkeit:</strong> {metadata_completeness:.1f}%</li>
                            </ul>
                        </div>
                        
                        <div class="metrics">
                            <h3>🏛️ Dokumentklassifikation</h3>
                            <ul>
                        ...
                        <div class="metrics">
                            <h3>💾 Backend-Schreibvorgänge</h3>
                            <p><strong>Vector Database:</strong> {vector_chunks} Chunks, {vector_keywords} Keywords</p>
                            <p><strong>Graph Database:</strong> {graph_relations} Relationen, {graph_entities} Entitäten</p>
...
- KI-Entitäten: {ai_entities} gefunden
- Metadata: {metadata_completeness:.1f}% vollständig
...
Backend-Schreibvorgänge:
- Vector DB: {vector_chunks} Chunks, {vector_keywords} Keywords
- Graph DB: {graph_relations} Relationen, {graph_entities} Entitäten
...
Covina System - {timestamp}
            ...
            "job_failed": MailTemplate(
                subject="❌ Covina Job fehlgeschlagen - {job_id}",
                html_body="""
                <html>
                <head>
                    <style>
                ...
                <body>
                    <div class="header">
                        <h2>⚠️ Dokumentenverarbeitung fehlgeschlagen</h2>
                    </div>
                    ...
                    <div class="content">
                        ...
                        <ul>
                            <li>Überprüfen Sie die Dateiformate (PDF, DOCX, TXT unterstützt)</li>
                            <li>Stellen Sie sicher, dass Dateien nicht beschädigt sind</li>
                            <li>Prüfen Sie die Dateigröße (max. 50MB pro Datei)</li>
...
- Dateiformate prüfen (PDF, DOCX, TXT)
- Dateien auf Beschädigungen prüfen
- Dateigröße beachten (max. 50MB)
...
Covina System - {timestamp}
            ...
            "system_alert": MailTemplate(
                subject="🚨 Covina System Alert - {alert_type}",
                html_body="""
                <html>
                <head>
                    <style>
...
class CovinaMailService:
    ...
    def __init__(self, config: MailConfig):
        ...
        logger.info(f"📧 Mail Service initialisiert - Server: {config.smtp_server}:{config.smtp_port}")
    ...
    async def send_email_async(
        ...
        try:
            ...
            if not template:
                logger.error(f"❌ Template nicht gefunden: {template_name}")
                return False
            ...
            logger.info(f"✅ E-Mail gesendet: {template_name} an {len(all_recipients)} Empfänger")
            return True
    ...
    def send_email_sync(
        ...
        try:
            # Verwende asyncio für synchronen Aufruf
            ...
            logger.error(f"❌ Synchroner E-Mail-Versand fehlgeschlagen: {e}")
            return False
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            logger.info(f"🔍 Teste SMTP-Verbindung zu {self.config.smtp_server}:{self.config.smtp_port}")
            ...
            await smtp.connect()
            logger.info("✅ SMTP-Verbindung hergestellt")
            
            # TLS/SSL Setup
            if self.config.use_tls:
                await smtp.starttls()
                logger.info("✅ TLS-Verschlüsselung aktiviert")
    ...
    async def send_email(
        ...
        try:
            ...
            logger.info(f"✅ E-Mail gesendet: \"{subject}\" an {len(all_recipients)} Empfänger")
            return True
...
# Factory für einfache Verwendung
...
def load_mail_config() -> MailConfig:
    """Lade Mail-Konfiguration aus Umgebungsvariablen oder Defaults"""
    return MailConfig(
        smtp_server=os.getenv("MAIL_SMTP_SERVER", "192.168.178.94"),
        smtp_port=int(os.getenv("MAIL_SMTP_PORT", 25)),
        username=os.getenv("MAIL_USERNAME", "system@fritz.box"),
        password=os.getenv("MAIL_PASSWORD", "v3f3b1d7"),
        ...
        sender_email=os.getenv("MAIL_SENDER_EMAIL", "system@fritz.box"),
        sender_name=os.getenv("MAIL_SENDER_NAME", "Covina System")
    )
