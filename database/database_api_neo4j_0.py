...
class Neo4jGraphBackend(GraphDatabaseBackend):
    ...
    def run_cypher(self, query: str, params: Dict = None) -> List[Dict]:
        ...
        # Use a driver-friendly wrapper that prefers execute_read/execute_write
        records = self._run_query(query, params or {}, write=False, single=False)
        out: List[Dict] = []
        for r in records or []:
            try:
                out.append(dict(r))
    ...
    def _run_query(self, query: str, params: Dict[str, Any] = None, *, write: bool = False, single: bool = False):
        """Run a Cypher query using the neo4j driver execute_read/execute_write pattern.

        Returns a single record if single=True else a list of records.
        """
        ...
        try:
            with self.driver.session(database=self.database) as session:
                # Prefer execute_write/execute_read to follow official examples and proper routing
                if write and hasattr(session, 'execute_write'):
                    records = session.execute_write(lambda tx: list(tx.run(query, params or {})))
                elif not write and hasattr(session, 'execute_read'):
