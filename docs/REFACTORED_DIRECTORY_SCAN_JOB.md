# Refactored DirectoryScanJob - Modulare Architektur
...
1. ✅ **Scanner Integration** (`ingestion.scanner.DirectoryScanner`)
2. ✅ **Handler System** (`ingestion.handlers.factory.HandlerFactory`)
3. ✅ **Archive Extraction** (`ArchiveIngestionHandler`)
4. ✅ **File Movement** (Copy to `data/uploads/scan_{id}/`)
5. ✅ **Recursive Processing** (Extracted files → new jobs)
...
class DirectoryScanJobModular:
    ...
    def __init__(
        ...
        self.handler_factory = handler_factory or create_default_factory()
    ...
    async def _scan_directory(self) -> List[FileEvent]:
    ...
    async def _copy_and_extract_files(self, file_events: List[FileEvent]) -> List[str]:
        """
    ...
    async def _create_and_submit_jobs(self, file_paths: List[str]):
        ...
        for chunk_idx, chunk in enumerate(file_chunks):
            ...
            def process_chunk_sync(job_id, file_paths_chunk, chunk_idx):
                ...
                try:
                    ...
                    loop.run_until_complete(
                        process_documents_batch(job_id, file_paths_chunk, Path(self.temp_dir))
                    )
            ...
            io_executor.submit(process_chunk_sync, upload_job_id, chunk, chunk_idx)
    ...
    async def _broadcast_status(self):
        ...
        await ws_manager.broadcast({
            ...
            "jobs_created": len(self.upload_jobs_created),
...
### **1. Replace DirectoryScanJob class** (Lines 237-520)

**Old:**
```python
class DirectoryScanJob:
    supported_extensions = {...}  # Manual list
    # Manual os.walk() implementation
...
```python
class DirectoryScanJob:
    # Uses DirectoryScanner ✅
    # Uses HandlerFactory ✅
    # Uses ArchiveIngestionHandler ✅
    # Copies files to temp_dir ✅
    # Extracts archives recursively ✅
```

### **2. Update supported_extensions** → Removed!

**Reason:** Now handled by `FileClassifier` in `ingestion/scanner.py`

### **3. Add temp_directory tracking**
...
upload_job_id = jm.create_job(
    len(chunk),
    temp_directory=str(self.temp_dir),  # ✅ Track temp directory
    scan_job_id=self.scan_job_id
)
```
...
- ❌ No file movement
- ❌ No modularity
- ❌ 290+ lines in single class

### **After (Modular):**
- ✅ `DirectoryScanner` handles scanning
- ✅ `FileClassifier` handles categorization
- ✅ `ArchiveIngestionHandler` handles extraction
- ✅ Files copied to `data/uploads/`
- ✅ Archives extracted recursively
- ✅ Modular, testable components
- ✅ ~150 lines (+ 440 lines in `ArchiveIngestionHandler`)

---

## 🎯 **Implementation Steps**

### **Step 1: Add Global Handler Factory** (After imports)
```python
# Global Handler Factory (after line ~65)
logger.info("🏗️ Initializing modular ingestion architecture...")
HANDLER_FACTORY = create_default_factory()
logger.info(f"✅ Handler Factory ready: {len(HANDLER_FACTORY.registry.snapshot())} handlers registered")
```

### **Step 2: Replace DirectoryScanJob Class** (Lines 237-520)
- Remove old implementation
- Add new `DirectoryScanJobModular` class
- Rename to `DirectoryScanJob` (keep same name for backward compatibility)

### **Step 3: Update /upload/directory endpoint** (No changes needed!)
- Endpoint already creates `DirectoryScanJob` correctly
- No changes required (backward compatible)

### **Step 4: Test Archive Support**
```python
# Test with ZIP file
...
# Expected:
# 1. 3 ZIP files found ✅
# 2. Each ZIP extracted ✅
# 3. Nested files discovered ✅
# 4. Jobs created for all files ✅
```
...
- [ ] Archive-Handler erstellt (`ingestion/handlers/archive.py`) ✅
- [ ] Handler in Factory registriert ✅
- [ ] Global Handler Factory in Backend ✅
- [ ] DirectoryScanJob refaktoriert ✅
- [ ] File Movement implementiert ✅
- [ ] Archive Extraction implementiert ✅
- [ ] Recursive Discovery implementiert ✅
- [ ] temp_directory tracking ✅
- [ ] Cleanup nach Erfolg ✅
...
**Next:** Replace old `DirectoryScanJob` with modular version
