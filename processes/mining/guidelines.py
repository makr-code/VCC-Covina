...
class RuleEvidence:
    ...
    Attributes:
        ...
        weight: Evidence weight/strength
        details: Additional context about this evidence
    """
    rule_id: str
    weight: float
...
class RuleEngine:
    """YAML-driven heuristics and mappings for process inference.
    
    The RuleEngine loads domain knowledge from YAML files to:
    - Normalize authority names and docket numbers
    - Extract signals from document metadata
    - Apply inference rules to infer process steps and transitions
    ...
    def __init__(self, yaml_text: Optional[str] = None, yaml_path: Optional[str] = None):
        """Initialize RuleEngine from YAML text or file.
    ...
    def normalize_authority(self, name: Optional[str]) -> Optional[str]:
        """Normalize authority name using configured patterns.
        
        Args:
            name: Raw authority name
            
        Returns:
            Normalized authority name or None
        """
        if not name:
            return None
    ...
    def normalize_aktenzeichen(self, text: Optional[str]) -> Dict[str, Any]:
        """Parse and normalize docket number using configured patterns.
        
        Args:
            text: Raw docket number
            
        Returns:
            Dictionary with parsed components (raw, series, number, year, etc.)
        """
    ...
    def infer_steps(self, signals: Dict[str, Any]) -> List[RuleEvidence]:
        """Infer process steps from signals using configured rules.
        
        Args:
            signals: Extracted document signals
            
        Returns:
            List of evidence for each matching step rule
        """
    ...
    def infer_transitions(self, signals: Dict[str, Any]) -> List[RuleEvidence]:
        """Infer process transitions from signals using configured rules.
        
        Args:
            signals: Extracted document signals
            
        Returns:
            List of evidence for each matching transition rule
        """
        evidences: List[RuleEvidence] = []
