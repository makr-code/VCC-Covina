...
Nutzt UDS3 Relational Backend für zentrale Verbindungsverwaltung.
...
class KnowledgeGapDB:
    """PostgreSQL-Datenbank für Knowledge Gap Management (via UDS3 Backend)"""
    
    def __init__(self, postgres_backend=None):
        """
        Initialize Knowledge Gap Database using UDS3 Relational Backend
        
        Args:
            postgres_backend: PostgreSQLRelationalBackend instance from UDS3
        """
        if postgres_backend is None:
            raise RuntimeError("postgres_backend is required. Pass UDS3 relational backend instance.")
    ...
    def _initialize_tables(self):
        """Create tables if they don't exist using UDS3 Backend"""
        try:
