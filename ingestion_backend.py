...
IO_WORKERS = int(os.getenv("WORKERS_IO", min(36, CPU_COUNT * 2)))  # Default: 36 (war 18)
CPU_WORKERS = int(os.getenv("WORKERS_CPU", min(36, CPU_COUNT * 2)))  # Default: 36 (war 18)
...
# Removed old log line - replaced with startup log in worker pool config section
        ...
        if job_manager.uds3_strategy.graph_backend:
            try:
                ...
                if hasattr(relations_core, 'driver') and relations_core.driver:
                    ...
                    logger.info(f"[OK] Neo4j: {document_id}")
                else:
...
async def process_documents_batch(
    ...
    try:
        ...
        # [OK] NEW: Verarbeite Dateien parallel mit job_id für File-Tracking
        tasks = [
            process_single_document(fp, jm, job_id=job_id)
            for fp in file_paths
        ]
        
        print(f"[OK] [BATCH] {len(tasks)} tasks created!\n", flush=True)
        print(f"[SYNC] [BATCH] Running asyncio.gather()...\n", flush=True)
...
async def start_chunked_upload(request: ChunkedUploadStartRequest):
    """
